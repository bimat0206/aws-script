...
# Function to get AWS Backup vaults
get_backup_vaults() {
...
# Get all AWS Backup vaults
backup_vaults=$(get_backup_vaults)
if [[ -z "$backup_vaults" ]]; then
    echo "No AWS Backup vaults found." >&2
    exit 1
fi

total_vaults=$(echo "$backup_vaults" | wc -w)
current_count=0

# Tag all AWS Backup vaults and show progress
for vault in $backup_vaults; do
    resource_arn="arn:aws:backup:$(aws configure get region):$AWS_ACCOUNT_ID:backup-vault:$vault"
    tag_resources "$resource_arn"
    ((current_count++))
    show_progress "$current_count" "$total_vaults"
done

echo -e "\nAll AWS Backup vaults have been tagged."
