...
# Function to get instance region
get_instance_region() {
    local instance_id=$1
    aws ec2 describe-instances --instance-ids "$instance_id" --query 'Reservations[0].Instances[0].Placement.AvailabilityZone' --output text | sed 's/\(.*\)[a-z]$/\1/'
}
...
# Tag all EC2 instances and show progress
for instance in $instances; do
    instance_region=$(get_instance_region "$instance")
    resource_arn="arn:aws:ec2:$instance_region:$AWS_ACCOUNT_ID:instance/$instance"
    tag_resources "$resource_arn"
    ((current_count++))
    show_progress "$current_count" "$total_instances"
done

echo -e "\nAll EC2 instances have been tagged."
